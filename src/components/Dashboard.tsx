...
export const Dashboard: React.FC = () => {
  ...
  const stats = [
    { label: 'Total de Livros', value: totalBooks, icon: BookOpen, color: 'text-blue-600', bg: 'bg-blue-100' },
    { label: 'Livros Disponíveis', value: availableBooks, icon: BookOpen, color: 'text-emerald-600', bg: 'bg-emerald-100' },
    { label: 'Empréstimos Ativos', value: activeLoans.length, icon: Clock, color: 'text-amber-600', bg: 'bg-amber-100' },
    { label: 'Atrasados', value: overdueLoans.length, icon: AlertTriangle, color: 'text-rose-600', bg: 'bg-rose-100' },
  ];
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map((stat, idx) => {
          ...
          // Map colors to frosted ones
          let colorClass = stat.color === 'text-blue-600' ? 'text-blue-400' : 
                           stat.color === 'text-emerald-600' ? 'text-emerald-400' : 
                           stat.color === 'text-amber-600' ? 'text-amber-400' : 'text-rose-400';
          let bgClass = stat.bg === 'bg-blue-100' ? 'bg-blue-500/20' : 
                        stat.bg === 'bg-emerald-100' ? 'bg-emerald-500/20' : 
                        stat.bg === 'bg-amber-100' ? 'bg-amber-500/20' : 'bg-rose-500/20';
          ...
          return (
            <div key={idx} className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-sm p-6 flex items-center gap-4 group hover:bg-white/10 transition-all">
              <div className={`w-12 h-12 rounded-full flex items-center justify-center border border-white/5 ${bgClass}`}>
          ...
          {activeLoans.length === 0 ? (
            ...
            <div className="space-y-4">
              {activeLoans.slice(0, 5).map(loan => {
                ...
                const isOverdue = new Date(loan.returnDate) < today;
                return (
